...
public class GlobalVars extends Application {
	...
	public static String composeMessageNumber;
	...
	public static String cameraVisionText = "";
	...
	public static void getVoiceRecognitionResult(String result) {
		if (result.contains("open message"))
			startActivity(Messages.class);
		else if (result.contains("open camera vision"))
			startActivity(CameraVison.class);
		else if (result.contains("open contacts"))
			startActivity(Contacts.class);
		else if (result.contains("open news"))
			startActivity(News.class);
		else if (result.contains("open main menu"))
			startActivity(Main.class);
		else if (result.contains("open voice recorder"))
			startActivity(VoiceRecorder.class);
		else if (result.contains("open music"))
			startActivity(MusicPlayer.class);
		else if (result.contains("create contact"))
			startActivity(ContactsCreate.class);
		else if (result.contains("make a call to")) {
			String phoneValue = result.split(",")[0].replaceAll("[^0-9]", "");
			callTo("tel:" + phoneValue);
		} else if (result.contains("send a message to")) {
			composeMessageNumber = result.split(",")[0].replaceAll("[^0-9]", "");

			GlobalVars.talk("Please tell me the message");

		} else if (result.contains("message body is")){
			composeMessage = result.split(",")[0].replace("[message body is", "");
		sendSMS(composeMessageNumber, composeMessage);
		}
